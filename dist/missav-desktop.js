...
(function() {
	...
	function findSidebar() {
		const box = document.querySelector("input[x-model=\"playlist.is_added\"]");
		if (!box) return null;
		const flex = box.closest("div.mb-5").parentElement?.parentElement;
		if (!flex) return null;
	...
	function playlistDock() {
		waitDOMContentLoaded(() => {
			...
			const undock = () => {
				const panel = findPanel();
				if (!panel) return;
				if (homeAnchor && homeAnchor.parentElement) homeAnchor.parentElement.insertBefore(panel, homeAnchor);
