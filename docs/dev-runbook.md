...
- **fast-save 自 v1.36.15 起是单一实现**：收藏/片单点击无条件拦截、请求一律由脚本发出（keepalive），Alpine 只作状态来源的首选（不再是拦截前提），反馈（跨标签 toast / saved-cache / 备份快照）乐观写在请求发出后。改它必须用 `cdp-press.ts` 真实输入复测勾选/回滚——片单 checkbox 的激活序列（pre-click 翻转 checked、取消后回滚、x-model effect 部分失效）是硬啃出来的，合成点击验证不出来
...
- **片单快照必须容忍"快照里还没有这个片单"**：上次备份之后新建的片单，快照里当然没有；回写时直接 `return` 会把该片单的勾选更新永久丢弃（实测用户账号：快照 34 个片单、面板 36 个，差的两个正是新建的）。补建条目要插**队首**——站点 `/cn/playlists` 列表页是新的在前，抓取顺序即快照顺序，插队首才和下次真备份一致
- **「建立片单」是整页 POST，且服务器会自动把当前番号加进新片单**：`/cn/playlists/create/<番号>` 提交后 302 到 `/cn/playlists/<新key>`，新片单里已经含该番号（实测 `gira67sq` 建完即含 `ap-684`，`/api/playlists/ap-684` 报 `is_added: true`）。这条链路脚本完全看不到（整页跳转、无 JS 钩子），所以**新建那一刻的加入不会立刻进快照**，要覆盖只能拦表单提交（未做）
