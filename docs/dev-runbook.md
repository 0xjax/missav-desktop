...
- **「是否收藏 / 是否已加入片单」的真值都在服务端，本地缓存只有一处**：收藏真值 = 站点 `/api/items/{id}/view`，片单勾选真值 = 站点 `/api/playlists/{番号}`；本地只有 `saved-cache`（图标秒显，0 请求，`/view` 回来后以服务器为准）。**快照不参与任何界面状态**（v1.36.22 起连回写也删了）——曾经的耦合是片单数量角标，已随功能移除
