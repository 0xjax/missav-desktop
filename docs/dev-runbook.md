...
- **WARNING 多源分段器是手动拉取，别改回自动**（v1.36.21 起）：缓存新鲜（7 天）直接渲染、**0 请求**；无新鲜缓存时只显示当前档一颗可点胶囊（`mx-seg-fetch`，title 提示"点击检测该番号的其他源"），**用户点了才发那 2 个整页请求**（`/{lang}/{裸番号}` + `/search/{番号}?filters=individual`）。理由：这两页是"用户没在看的页面"，最像爬虫，是 Cloudflare 人机验证与限速的主要来源（备份自动爬取同理，已一并移除）。实测点击后正好发这 2 个请求、拉完写缓存、失败时 toast 并退回可点单档
