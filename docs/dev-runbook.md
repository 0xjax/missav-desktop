...
- **fast-save 自 v1.36.15 起是单一实现**：收藏/片单点击无条件拦截、请求一律由脚本发出（keepalive），Alpine 只作状态来源的首选（不再是拦截前提），反馈（跨标签 toast / saved-cache）乐观写在请求发出后。改它必须用 `cdp-press.ts` 真实输入复测勾选/回滚——片单 checkbox 的激活序列（pre-click 翻转 checked、取消后回滚、x-model effect 部分失效）是硬啃出来的，合成点击验证不出来
- **「是否收藏 / 是否已加入片单」的真值都在服务端，本地缓存只有一处**：收藏真值 = 站点 `/api/items/{id}/view`，片单勾选真值 = 站点 `/api/playlists/{番号}`；本地只有 `saved-cache`（图标秒显，0 请求）。**它是双向自校正的**：`/view` 返回前用缓存点亮（只对"本浏览器操作过"的番号有条目），`/view` 返回后把服务端真值写回缓存——少了回写这一步，在别处取消过收藏的番号会在这台机器上每次加载都闪一次错值，且那条 `true` 永远不被覆盖（v1.36.23 修）。**快照不参与任何界面状态**（v1.36.22 起连回写也删了）——曾经的耦合是片单数量角标，已随功能移除
- **备份快照只由抓取产生，别再加自动爬取或本地回写**（v1.36.22 决策）：自动备份每 3 天约 130 个请求，是 Cloudflare 人机验证的主源；回写则是"请求发出后不等响应"的乐观写，没有自动重爬纠正就会把漂移永久固化进快照，导出结果不可信。要新数据就让用户手动点「立即备份」
...
- **WARNING 多源分段器是手动拉取，别改回自动**（v1.36.21 起）：缓存新鲜（7 天）直接渲染、**0 请求**；无新鲜缓存时只显示当前档一颗胶囊（v1.36.27 起为 `mx-seg-guess`：虚线描边 + 无填充 + 标签带 `?`，title 说明"档位为推测"），**用户点了才发那 2 个整页请求**（`/{lang}/{裸番号}` + `/search/{番号}?filters=individual`）。理由：这两页是"用户没在看的页面"，最像爬虫，是 Cloudflare 人机验证与限速的主要来源（备份自动爬取同理，已一并移除）。实测点击后正好发这 2 个请求、拉完写缓存、失败时 toast 并退回可点单档
- **`mx-seg-guess`（未检测）与 `mx-seg-locked`（已检测·仅此单源）必须一眼可分**：曾经的差别只有透明度 1 / 0.85 + 光标，实测分不出来（用户反馈"不知道是预测还是结果"）。现在靠**虚实 + 有无 `?`** 区分；kind 色是为实色填充挑的，当描边画在深色顶栏上太暗（原版 `#4c566a`），故 guess 态统一 `filter: brightness(1.7)` 提亮
- **分段器加载态沿用同一个框**（`mx-seg-loading`：虚线框 + 档位色 + 居中转圈，44×22 与常态档位一致）。旧版是清空容器类名再画一颗灰胶囊（`mx-seg-skeleton`），整个框消失再回来，前后断裂。WARNING 空按钮转 flex 容器后 `.mx-seg` 的 `line-height` 不再撑高，`min-height` 必须写**总高 22**（站点 Tailwind preflight 是 border-box；写 18 时实测只有 18 高）
- **兄弟源列表以站点版本菜单为准，搜索页只供徽章**：菜单 `[aria-labelledby=download-option-menu-button]` 只在裸番号主条目页的 SSR 里（实测每个番号都有裸页），列的是全部兄弟源（含跨语言字幕版）。搜索页会按语言过滤，而且**部分番号的搜索页一张匹配卡片都不返回**（实测 sdmf-009 / har-050 / umd-971 / har-068）——只用搜索页会漏兄弟源，表现为分段器只剩当前档。**两个请求都要发**：菜单一定包含裸番号本身，而裸番号的类型只能靠搜索页徽章判定（跳过搜索页会把 fneo-014 这类"裸 id 其实是字幕版"误标成原版）
...
- **WARNING 后台标签页收不到真实输入**：`document.visibilityState === 'hidden'` 的 tab，Chrome 直接丢弃 `Input.dispatchMouseEvent`——埋点日志为空、状态不变，看着像"点了没反应"（实测踩坑）。`cdp-press.ts` 现在按压前先 `Page.bringToFront`，手写 CDP 脚本同样要加
- **「建立片单」是整页 POST，且服务器会自动把当前番号加进新片单**：`/cn/playlists/create/<番号>` 提交后 302 到 `/cn/playlists/<新key>`，新片单里已经含该番号（实测 `gira67sq` 建完即含 `ap-684`，`/api/playlists/ap-684` 报 `is_added: true`）。这条链路脚本完全看不到（整页跳转、无 JS 钩子）——想在这方面做反馈只能拦表单提交（未做）
- **片单面板的 Alpine 项就是 `{is_added, key, name}`**：补建快照条目用的片单名直接取 `item.name`（DOM `label` 兜底即可，不必再查接口）
